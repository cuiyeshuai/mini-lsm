...
/// In MVCC reads/compaction the exposed key is (user key, ts): a@9 and a@4
/// remain distinct. In a transaction overlay the key is just user bytes, so a
/// private update overrides the snapshot's a regardless of its commit timestamp.
